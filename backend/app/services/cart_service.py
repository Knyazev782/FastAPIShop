...
from ..shcemas.cart import CartResponse, CartItem, \
...
class CartService:
    ...
    def update_cart_item(self, cart_data: Dict[int, int], item: CartItemCreate) -> Dict[int, int]:
        if item.product_id not in cart_data:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Product with id {item.product_id} not found is cart"
            )
        cart_data[item.product_id] = item.quantity
        return cart_data
    ...
    def remove_from_cart(self, cart_data: Dict[int, int], product_id: int) -> Dict[int, int]:
        if product_id not in cart_data:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f'Product with if {product_id} not found in cart'
            )
    ...
    def get_cart_details(self, cart_data: Dict[int, int]) -> CartResponse:
        ...
        for product_id, quantity in cart_data.items():
            if product_id in products_dict:
                ...
                cart_items = CartItem(product_id=product_id, name=product.name,
                                      price=product.price, quantity=quantity, subtotal=subtotal, image_url=product.image_url)
