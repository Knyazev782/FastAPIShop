...
class CartItemsBase(BaseModel):
    ...
    quantity: int = Field(..., gt=0, description="Quantity be greater then 0")


class CartItemCreate(CartItemsBase):
    pass

class CartItemUpdate(BaseModel):
    product_id: int = Field(..., description="Product ID")
    quantity: int = Field(..., gt=0, description="New quantity must be then 0")
